...
export function validateNewsSecrets(value) {
  ...
  if (Object.keys(value).some(key => !['NEWS_INGEST_TOKEN', 'NEWS_EDITOR_TOKEN', 'NEWS_REVIEW_TOKEN', 'NEWS_RESEARCH_TOKEN'].includes(key))) throw new Error('News secrets file contains an unknown credential.');
  ...
  if (value.NEWS_RESEARCH_TOKEN !== undefined && (typeof value.NEWS_RESEARCH_TOKEN !== 'string' || value.NEWS_RESEARCH_TOKEN.length < 32 || [NEWS_INGEST_TOKEN, NEWS_EDITOR_TOKEN, value.NEWS_REVIEW_TOKEN].includes(value.NEWS_RESEARCH_TOKEN))) throw new Error('Use a separate random research secret of at least 32 characters.');
}
