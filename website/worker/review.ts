...
import { researchBoard } from "./research";
...
export async function reviewRequest(request: Request, env: ReviewEnv): Promise<Response | null> {
  ...
  try {
    ...
    if (path === BASE + "/board" && request.method === "GET") {
      ...
      return reply({ summary: pack.run_summary, coverage: pack.coverage_and_gaps, ...ranked, decisions: decisions.results, research: await researchBoard(env.DB, pack.evidence_items.map(i => i.item_id)) });
    }
