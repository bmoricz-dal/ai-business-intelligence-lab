...
def page_frame(canvas, doc) -> None:
    draw_page_frame(canvas, doc, page_w=PAGE_W, page_h=PAGE_H, left=LEFT, right=RIGHT, brand=BRAND, short_title="Accounting AI readiness 2026", footer_note="Secondary evidence | Definitions and denominators remain separate")


def cover(canvas, doc) -> None:
    draw_signature_cover(canvas, page_w=PAGE_W, page_h=PAGE_H, left=LEFT, right=RIGHT, brand=BRAND, series="Accounting sector / Current state", formal_title="UK Accounting SMEs / AI readiness / 2026", headline="AI use has started. Operational readiness is still forming.", subtitle="A five-dimension current-state report using secondary evidence only.", report_date=REPORT_DATE, author=AUTHOR, taxonomy="Adoption / Integration / Governance / Use cases / Pathways")
